...
int io_write(resmgr_context_t *ctp, io_write_t *msg, RESMGR_OCB_T *ocb) {
	...
	for (i = 0; i < strlen(device_buffer); i++){ //check if valid input
		if(strncmp(buffer_pointer, "mix on ", strlen("mix on")) == 0){
			strcpy(device_status, "On");
			buffer_pointer += strlen("mix on"); //increment by the size of the string that was matched
			int x = atoi(buffer_pointer);
		}
		else if(strncmp(buffer_pointer, "mix off " , strlen("mix off")) == 0){
			strcpy(device_status, "Off");
			buffer_pointer += strlen("mix off ");
		}
		else if(strncmp(buffer_pointer, "pump on ", strlen("pump on ")) == 0){
			buffer_pointer += strlen("pump on ");
			int x = atoi(buffer_pointer); //x is the mixer number
				printf("pump on %d", x);
		}
		else if(strncmp(buffer_pointer, "pump off ", strlen("pump off ")) == 0){
			buffer_pointer += strlen("pump off ");
			int x = atoi(buffer_pointer); //x is the mixer number
				printf("pump off %d", x);
		}
		else if(strncmp(buffer_pointer, "full ", strlen("full ")) == 0){
			buffer_pointer += strlen("pump on ");
			int x = atoi(buffer_pointer); //x is the mixer number
				printf("pump on %d", x);
		}
		else if(strncmp(buffer_pointer, "empty ", strlen("empty ")) == 0){
			buffer_pointer += strlen("empty ");
			int x = atoi(buffer_pointer); //x is the mixer number
				printf("empty %d", x);
		}
		else{
